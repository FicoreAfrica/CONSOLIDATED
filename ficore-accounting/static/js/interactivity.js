document.addEventListener('DOMContentLoaded', function() {
    ...
    // Dark Mode Toggle (merged with base.html's toggleDarkMode)
    ...
    if (toolsLink) {
        toolsLink.addEventListener('click', function(event) {
            ...
            if (toolsSection) {
                ...
                console.warn('Tools section not found, skipping navigation');
            }
        });
    }
    ...
    flashMessages.forEach(message => {
        const duration = 3 * 1000; // Reduced to 3s
        const end = Date.now() + duration;
        const colors = ['#ff0a54', '#ff477e', '#ff7096', '#ff85a1', '#fbb1bd', '#f9bec7'];

        (function frame() {
            confetti({
                particleCount: 5, // Reduced for performance
                angle: 60,
                spread: 70,
                origin: { x: 0 },
                colors: colors
